import {BffFilterParam, ProjectDescriptionData} from "../../types/Api/dataTypes";

export const projectsList: ProjectDescriptionData[] = [
    {
        id: "project_1",
        title: "общественное пространство «среда»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: true,
        locale: "ru",
        images: [
            {
                alt: "project_1_img_1",
                src: "/project_1_img_1.png",
                projectPreview: true,
                showOrder: 0,
            },
            {
                alt: "project_1_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_1_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_2",
        title: "общественное пространство «ведомство»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: true,
        locale: "ru",
        images: [
            {
                alt: "project_2_img_1",
                src: "/project_2_img_1.png",
                projectPreview: true,
                showOrder: 0,
            },
            {
                alt: "project_2_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_2_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_3",
        title: "ревитализация территории завода",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: true,
        locale: "ru",
        images: [
            {
                alt: "project_3_img_1",
                src: "/project_3_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_3_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_3_img_3",
                src: "/project_2_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_4",
        title: "общественное пространство «среда»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_4_img_1",
                src: "/project_1_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_4_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_4_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_5",
        title: "общественное пространство «ведомство»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_5_img_1",
                src: "/project_2_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_5_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_5_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_6",
        title: "ревитализация территории завода",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_6_img_1",
                src: "/project_3_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_6_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_6_img_3",
                src: "/project_1_img_1.png",
                showOrder: 2,
            },
            {
                alt: "project_6_img_4",
                src: "/project_2_img_1.png",
                showOrder: 3,
            },
            {
                alt: "project_6_img_5",
                src: "/project_1_img_1.png",
                showOrder: 4,
            },
            {
                alt: "project_6_img_6",
                src: "/project_2_img_1.png",
                showOrder: 5,
            },
            {
                alt: "project_6_img_7",
                src: "/project_1_img_1.png",
                showOrder: 6,
            },
            {
                alt: "project_6_img_8",
                src: "/project_2_img_1.png",
                showOrder: 7,
            },
            {
                alt: "project_6_img_9",
                src: "/project_1_img_1.png",
                showOrder: 8,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_7",
        title: "общественное пространство «среда»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_1_img_1",
                src: "/project_1_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_1_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_1_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_8",
        title: "общественное пространство «ведомство»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_2_img_1",
                src: "/project_2_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_2_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_2_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_9",
        title: "ревитализация территории завода",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_3_img_1",
                src: "/project_3_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_3_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_3_img_3",
                src: "/project_2_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_10",
        title: "общественное пространство «среда»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_4_img_1",
                src: "/project_1_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_4_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_4_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_11",
        title: "общественное пространство «ведомство»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_5_img_1",
                src: "/project_2_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_5_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_5_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_12",
        title: "ревитализация территории завода",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_6_img_1",
                src: "/project_3_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_6_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_6_img_3",
                src: "/project_1_img_1.png",
                showOrder: 2,
            },
            {
                alt: "project_6_img_4",
                src: "/project_2_img_1.png",
                showOrder: 3,
            },
            {
                alt: "project_6_img_5",
                src: "/project_1_img_1.png",
                showOrder: 4,
            },
            {
                alt: "project_6_img_6",
                src: "/project_2_img_1.png",
                showOrder: 5,
            },
            {
                alt: "project_6_img_7",
                src: "/project_1_img_1.png",
                showOrder: 6,
            },
            {
                alt: "project_6_img_8",
                src: "/project_2_img_1.png",
                showOrder: 7,
            },
            {
                alt: "project_6_img_9",
                src: "/project_1_img_1.png",
                showOrder: 8,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_13",
        title: "общественное пространство «среда»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_1_img_1",
                src: "/project_1_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_1_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_1_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_14",
        title: "общественное пространство «ведомство»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_2_img_1",
                src: "/project_2_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_2_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_2_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_15",
        title: "ревитализация территории завода",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_3_img_1",
                src: "/project_3_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_3_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_3_img_3",
                src: "/project_2_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_16",
        title: "общественное пространство «среда»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_4_img_1",
                src: "/project_1_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_4_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_4_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_17",
        title: "общественное пространство «ведомство»",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_5_img_1",
                src: "/project_2_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_5_img_2",
                src: "/project_1_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_5_img_3",
                src: "/project_3_img_1.png",
                showOrder: 2,
            },
        ],
        tags: [

        ]
    },
    {
        id: "project_18",
        title: "ревитализация территории завода",
        location: "санкт-петербург",
        yearOfBuild: "2020",
        status: "проектирование",
        assignment: "общественное пространство",
        description: "Эстетически брутализм вписывается в контекст европейского модернизма 1950—1970-х годов (в сфере пластических искусств — в кино, фотографии, графике, скульптуре, оформлении интерьеров) — с его поисками новых средств выразительности. Это, в частности, интерес к локальному цвету, пластичной броской «модернистской» форме, остро-выразительным фактурам. Декларативной лёгкости и приглаженности «интернационального стиля» брутализм противопоставил впечатляющую мощь конструкций и объёмов, смелые крупномасштабные композиционные решения. Например, одна из популярных тем у бруталистов, часто используемая в проектах административных и общественных зданий — это ступенчатая пирамида, перевёрнутая основанием вверх и приподнятая над землёй на пилонах (Посольство Великобритании в Риме, 1971 г., архитектор сэр Бэзил Спенс; Бостон Сити Холл, Бостон, Массачусетс, США, 1981, архитекторы: Г. Кальман, М. Мак-Киннель, Э. Ноулс и другие). \n Подобные идеи вдохновлялись верой в новые строительные технологии, в частности, в неисследованные ещё возможности такого нового строительного материала, как армированный бетон. Пластичность бетона, его «скульптурные» возможности были едва ли не главным элементом художественного языка у архитекторов-необруталистов.",
        mainPagePreview: false,
        locale: "ru",
        images: [
            {
                alt: "project_6_img_1",
                src: "/project_3_img_1.png",
                showOrder: 0,
                projectPreview: true,
            },
            {
                alt: "project_6_img_2",
                src: "/project_2_img_1.png",
                showOrder: 1,
            },
            {
                alt: "project_6_img_3",
                src: "/project_1_img_1.png",
                showOrder: 2,
            },
            {
                alt: "project_6_img_4",
                src: "/project_2_img_1.png",
                showOrder: 3,
            },
            {
                alt: "project_6_img_5",
                src: "/project_1_img_1.png",
                showOrder: 4,
            },
            {
                alt: "project_6_img_6",
                src: "/project_2_img_1.png",
                showOrder: 5,
            },
            {
                alt: "project_6_img_7",
                src: "/project_1_img_1.png",
                showOrder: 6,
            },
            {
                alt: "project_6_img_8",
                src: "/project_2_img_1.png",
                showOrder: 7,
            },
            {
                alt: "project_6_img_9",
                src: "/project_1_img_1.png",
                showOrder: 8,
            },
        ],
        tags: [

        ]
    },
]

export const mockBffServices: BffFilterParam[] = ["ревитализация", "дизайн", "градостроительство", "освещение"];

export const mockBffAssignments: BffFilterParam[] = ["общественное пространство", "архитектура", "интерьер", "объект культурного наследия"];

export const mockBffYearsOfBuilds: BffFilterParam[] = ["2019", "2020", "2021", "2022"];