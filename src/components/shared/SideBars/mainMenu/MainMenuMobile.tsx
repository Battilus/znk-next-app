...
const MainMenuMobile: FC<IProps> = (
  ...
  const dropdownHandler = useCallback((drop?: boolean) => {
    setDropdown(Boolean(drop));
  }, []);

  const selectDropdownStyle = () => {
    if (isHomeLocation) {
      return dropdown ? 'h-[438px]' : 'h-[307px]';
    }

    return dropdown ? 'h-[496px]' : 'h-[365px]';
  }
  ...
  return (
    <div className="fixed right-0 top-0 z-99">
      ...
      <div
        className={`flex flex-col ${selectDropdownStyle()} w-full fixed left-0 top-0 
